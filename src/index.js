...
import { getPropByPath, objectAssign } from './utils';
...
export default {
    ...
    created() {
        ...
        column.renderCell = function (h, data) {
            if (_self.$scopedSlots.default && data.column.showSlot) {
                // fixed 下的 slot 重复问题
                if (data.column.fixed === data.tableFixed ||
                    (data.column.fixed === true && data.tableFixed === 'left') ||
                    (!data.column.fixed && !data.tableFixed)) {
